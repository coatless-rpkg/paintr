...
# ---------------------------------------------------------------------------
#
# **A BASE PLOT IS NOT DRAWN ONCE. IT IS RE-DRAWN FROM A RECORDING, ONTO A DEVICE
# IT HAS NEVER SEEN, AND THE PACKAGE'S PRIMARY MEDIUM IS THAT RE-DRAW.**
#
# `recordPlot()` snapshots the graphics engine's display list -- the R-level calls,
# with their arguments already evaluated -- and `replayPlot()` re-executes them on
# whatever device is current. That is how `pkgdown` renders an `@examples` block,
# how `dev.copy()`/`dev.print()` work, and how an RStudio plot pane repaints when
# the user drags its edge.
#
# MEASURED. `evaluate::evaluate()` -- which pkgdown reaches through `downlit` with
# `new_device = TRUE` -- opens `ragg::agg_record()`, and its default is
# **6.667 x 6.667in**. pkgdown then replays onto its figure device, whose default is
# **7.292 x 4.507in**. Different size AND different aspect, every time.
#
# On that replay, `par(mai =)`, `plot.new()` and `plot.window(asp = 1)` are all
# re-executed against the new device, so `par("usr")` re-letterboxes and every
# RECTANGLE follows it exactly: `ux(xl)` is `-x0/u + (x0 + u*edge)/u`, i.e. `edge`
# -- a pure layout-unit coordinate, with the device divided out. The rectangles were
# always replay-invariant BY CONSTRUCTION.
#
# THE TEXT WAS NOT, in two independent ways, and both were measured on
# `paint_list(list(a = 1:4, b = "x", c = c(TRUE, FALSE, NA)))`:
#
#   * ITS SIZE. `text(cex =)` is a multiple of `par("ps")` -- a size in POINTS. A
#     point does not scale with the device. Text fitted to a 1.079in cell was
#     replayed, still 24pt, into a 0.663in cell.
#   * ITS ANCHOR. `dx_sig` is in inches and is divided by the RECORDING device's
#     `usr_per_in`. That ratio is only invariant while the fit is linear in `u`, and
#     `max_pt` breaks the linearity the moment it binds -- which it does on every
#     small structure. Recorded `dx_sig/u = -0.025`; the correct value on the replay
#     device was `-0.607`. The `FALSE` glyph landed 55px past the right edge of its
#     own cell.
#
# NO ARRANGEMENT OF NUMBERS FIXES THIS. Base graphics has no font size in user
# units, so a picture whose text is baked into the display list cannot rescale with
# the panel. The size has to be CHOSEN AGAIN, on the device that is finally being
# drawn on.
#
...
render_base <- function(cells, col_w, n_row,
  ...
  # device) `resolved` is exactly what the old code returned.
  resolved <- grDevices::recordGraphics(
    {
      ...
      res$usr <- graphics::par("usr")
      res$pin <- graphics::par("pin")
      # The chrome, exactly as `draw_bands()` was handed it: `NULL` for a band that
      # was not drawn, the string for one that was.
      res$graph_title <- graph_title
      res$graph_subtitle <- graph_subtitle
      res$note <- note
      res
    },
    list(),
  ...
  # device's numbers.
  #
  # The warning fires ONCE, HERE, on the record device -- deliberately OUTSIDE the
  # deferred expression. A base plot used to draw exactly once, but a recorded one
  # re-draws on every resize, page refresh and `dev.copy()`; warning inside the
  # expression would turn one warning into the same torrent `warn_floor_once()`
  # exists to stop in grid. `resolved` carries the record-device fit, so the numbers
  # are honest for the device the plot is first made on. `floor_message()` is owned
  # by `R/warn.R`, the one definition both renderers share.
