...
one part of it: *this* row, *that* column, the cells that pass a test. Every
...
`highlight_area` wants a logical structure the same shape as your data: `TRUE`
wherever a cell should light up, `FALSE` everywhere else. Nothing more. Here is a
...
`highlight_rows()` takes a row selection: integer positions, or a logical
...
This is the quickest way to show a class what a vectorised comparison *is*: the
red-and-white grid of `m > 4` and the highlighted picture are the same object.
...
Here is the invariant, stated plainly: **if a painter can draw a structure,
...
above were base graphics; here is the identical highlight through `gpaint_*`:
