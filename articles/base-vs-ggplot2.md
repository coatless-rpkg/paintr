...
family draws with base graphics, while the `gpaint_*()` family returns a
ggplot2 object. This article is about which one to reach for, and about
one real difference between them that is easy to trip over if you lean
on the ggplot side too hard.
...
unchanged, including `show_indices`, `highlight_area`, `sigfig`, and the
rest. Whichever family a structure has a `paint_*()` for, it has a
matching `gpaint_*()`.
...
output. There is nothing to print and nothing to keep, so you call it
and the drawing appears.
...
## The ggplot is a shell

A `gpaint_*()` object is a real ggplot object, but its entire panel is
drawn by a custom grid grob, dropped in as a single annotation over a
meaningless `0..1` coordinate system. There is no `aes()` mapping, no
geom, and no scale carrying any of the meaning you see.

The practical consequence is that the object does **not** compose the
way a normal ggplot does:

- `ggplot_build()` sees an empty layer, because there is no data behind
  the panel to build.
...
First, the cell text is fitted to the device at *draw* time, but ggplot2
...
and the cells compose because they were never ggplot layers to begin
with.

## The short version

Use `paint_*()` when you want a picture drawn now, at the console, in a
script, or in a chunk. Use `gpaint_*()` when you want a ggplot object to
