...
someone *see* the shape of the thing they are working with: that a
vector is a row of boxes, that a data frame is a stack of columns that
happen to line up, that an array is a pile of matrices. paintr draws
...
The two draw the same picture. One honest caveat about the ggplot
...
If the matrix has dimnames, they draw in the margins; `show_dimnames`
takes a character vector over `"none"`, `"row"`, `"column"`, and `"all"`
to choose which.

## Data frames

A data frame is the workhorse, and it gets the richest default picture:
a header row of column names, the type of each column beneath it, and a
row-name gutter whenever the row names are meaningful.
...
row names, so the gutter appears; a fresh `iris` would not draw one.
Toggle the extra rows with `show_names`, `show_types`, and
`show_rownames`.
...
A list is the flexible container: its elements can have different
...
Here is the idea worth pausing on. A data frame is just a list whose
elements all share a length, so when the elements *do* line up, the list
draws as a clean rectangle. Break one length and the rectangle breaks
with it, ragged edge and all. That ragged edge is the difference between
a list and a data frame, drawn literally.

paintr does not draw nesting: a list inside a list collapses to a grey
`<list [2]>` token rather than recursing. Use `show_indices = "cell"` to
label each cell with its `[[j]][i]` accessor.
...
4\].](paintr_files/figure-html/unnamed-chunk-12-1.png)

Each block is one slice; the label under a cell is its full subscript,
`[2, 3, 4]`, a genuine 3-D index into the array. `show_dimnames` works
as it does for a matrix, with an added `"slice"` lane. A genuinely huge
array elides its middle, the same way the other structures do once they
pass their size caps.
...
to stay readable. Each block keeps its own honest slice title, so
wrapping is just reading order.
...
one cramped row.](paintr_files/figure-html/unnamed-chunk-13-1.png)
...
- [`vignette("teaching", package = "paintr")`](https://r-pkg.thecoatlessprofessor.com/paintr/articles/teaching.md)
  puts these pictures to work in actual lesson material.
